...
import type {
  CreateLeadCommand,
  LeadListFilters,
  RecordLeadResultCommand,
  UpdateLeadSettingsCommand,
} from './contracts';
import { LeadDomainError } from './errors';

const LEAD_FIELDS =
  'id, contact_id, source, external_id, campaign_id, campaign_name, form_id, form_name, company, plan, opportunity_type_id, catalog_item_id, status, priority, assigned_to_user_id, received_at, attempt_count, dni_last4, discard_reason_id, closed_at, sold_product, won_amount, won_currency, created_at, updated_at, opportunity_type:opportunity_types(id,name,color), catalog_item:catalog_items(id,name,sku,kind,price,currency), contact:contacts(id, name, phone, email, company, avatar_url), pending_tasks:lead_tasks(id, assigned_to_user_id, status, due_at, created_at)';
...
export interface LeadRepository {
  ...
  recordResult(
    accountId: string,
    leadId: string,
    command: RecordLeadResultCommand
  ): Promise<unknown>;
  listDiscardReasons(
    accountId: string,
  ...
  updateSettings(
    accountId: string,
    command: UpdateLeadSettingsCommand
  ): Promise<unknown>;
}
...
function fail(
  ...
  if (error?.code === '23514' || error?.code === '23505')
    throw new LeadDomainError(
      'CONFLICT',
      'The lead operation conflicts with its current state'
    );
  if (error?.code === '42501')
    throw new LeadDomainError('FORBIDDEN', 'Insufficient lead permissions');
  console.error(`[leads:${operation}] database error`, { code: error?.code });
  throw new LeadDomainError(
    'DATA_ACCESS_ERROR',
    'Could not complete the lead operation'
  );
}
...
export class SupabaseLeadRepository implements LeadRepository {
  ...
  async create(accountId: string, command: CreateLeadCommand) {
    const { data, error } = await this.db.rpc('create_lead_with_initial_task', {
      ...
      p_received_at: command.receivedAt ?? new Date().toISOString(),
      p_assigned_to_user_id: command.assignedToUserId ?? null,
      p_next_follow_up_at: command.nextFollowUpAt,
      p_campaign_id: command.campaignId ?? null,
      p_campaign_name: command.campaignName ?? null,
      p_form_id: command.formId ?? null,
      p_form_name: command.formName ?? null,
      p_company: command.company ?? null,
      p_plan: command.plan ?? null,
      p_priority: command.priority,
    });
    if (error) fail('create', error);
    const leadId = data as string;
    if (command.opportunityTypeId || command.catalogItemId) {
      const { error: updateError } = await this.db
        .from('leads')
        .update({
          opportunity_type_id: command.opportunityTypeId ?? null,
          catalog_item_id: command.catalogItemId ?? null,
        })
        .eq('account_id', accountId)
        .eq('id', leadId);
      if (updateError) fail('create_commercial', updateError);
    }
    const values = Object.entries(command.fieldValues ?? {});
    if (values.length) {
      const { error: valuesError } = await this.db
      ...
      if (valuesError) fail('create_field_values', valuesError);
    }
    return leadId;
  }
  ...
  async recordResult(
    ...
    const { data, error } = await this.db.rpc('record_lead_result', {
      ...
      p_next_follow_up_at: command.nextFollowUpAt ?? null,
      p_discard_reason_id: command.discardReasonId ?? null,
      p_assigned_to_user_id: command.assignedToUserId ?? null,
      p_reason_code: command.reasonCode ?? null,
      p_sold_product: command.soldProduct ?? null,
      p_won_amount: command.wonAmount ?? null,
      p_won_currency: command.wonCurrency ?? null,
    });
  ...
  async getSettings(accountId: string) {
    const { data, error } = await this.db
      ...
      .select('lead_config, feature_flags')
      .eq('account_id', accountId)
      .maybeSingle();
    if (error) fail('get_settings', error);
    if (!data)
      throw new LeadDomainError('NOT_FOUND', 'Lead settings not found');
    return data;
  }

  async updateSettings(accountId: string, command: UpdateLeadSettingsCommand) {
    const current = (await this.getSettings(accountId)) as {
      lead_config?: Record<string, unknown>;
      feature_flags?: Record<string, boolean>;
    };
    const leadConfig = { ...(current.lead_config ?? {}) };
    if (command.closeNoResponseAfter !== undefined)
      leadConfig.close_no_response_after = command.closeNoResponseAfter;
    if (command.suggestFollowUp !== undefined)
      leadConfig.suggest_follow_up = command.suggestFollowUp;
    if (command.requireNextStep !== undefined)
      leadConfig.require_next_step = command.requireNextStep;
    const { data, error } = await this.db
      .from('account_settings')
      .update({ lead_config: leadConfig })
      .eq('account_id', accountId)
      .select('lead_config, feature_flags')
      .single();
