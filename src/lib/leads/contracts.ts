...
const STATUSES: LeadStatus[] = ["new", "in_progress", "follow_up", "won", "discarded"];
const PRIORITIES: LeadPriority[] = ["low", "normal", "high", "urgent"];
const CHANNELS: LeadActivityChannel[] = ["whatsapp", "phone", "email", "other", "system"];
const RESULTS: LeadActivityResult[] = ["no_answer", "contacted", "qualified", "won", "discarded", "rescheduled", "note", "assigned"];
...
function object(value: unknown): Record<string, unknown> {
  if (!value || typeof value !== "object" || Array.isArray(value)) throw validationError("Request body must be a JSON object");
  return value as Record<string, unknown>;
}

function optionalText(value: unknown, field: string, max = 500): string | undefined {
  if (value == null || value === "") return undefined;
  if (typeof value !== "string") throw validationError(`${field} must be a string`, { [field]: "invalid_type" });
  const result = value.trim();
  if (!result || result.length > max) throw validationError(`${field} must contain between 1 and ${max} characters`, { [field]: "invalid_length" });
  return result;
}

function uuid(value: unknown, field: string, optional = false): string | undefined {
  if (optional && (value == null || value === "")) return undefined;
  if (typeof value !== "string" || !UUID.test(value)) throw validationError(`${field} must be a valid UUID`, { [field]: "invalid_uuid" });
  return value;
}

function date(value: unknown, field: string, optional = false): string | undefined {
  if (optional && (value == null || value === "")) return undefined;
...
function enumValue<T extends string>(value: unknown, field: string, values: readonly T[], fallback?: T): T {
  ...
  if (typeof value !== "string" || !values.includes(value as T)) throw validationError(`${field} has an unsupported value`, { [field]: "invalid_value" });
  return value as T;
}
...
export function parseLeadListFilters(params: URLSearchParams): LeadListFilters {
  ...
  if (!Number.isInteger(page) || page < 1) throw validationError("page must be a positive integer", { page: "invalid_value" });
  if (!Number.isInteger(limit) || limit < 1 || limit > 100) throw validationError("limit must be between 1 and 100", { limit: "invalid_value" });
  const assigned = params.get("assigned_to");
  return {
    status: params.has("status") ? enumValue(params.get("status"), "status", STATUSES) : undefined,
    priority: params.has("priority") ? enumValue(params.get("priority"), "priority", PRIORITIES) : undefined,
    assignedToUserId: assigned === "unassigned" ? null : uuid(assigned, "assigned_to", true),
...
export function parseRecordResult(value: unknown): RecordLeadResultCommand {
  ...
  const command: RecordLeadResultCommand = {
    channel: enumValue(body.channel, "channel", CHANNELS), result,
...
export function parseAssignment(value: unknown): RecordLeadResultCommand {
  const body = object(value);
  return { channel: "system", result: "assigned", assignedToUserId: uuid(body.assigned_to_user_id, "assigned_to_user_id")!, note: optionalText(body.note, "note", 4000) };
}
...
export function parseLeadSettings(value: unknown): UpdateLeadSettingsCommand {
  ...
  if (body.close_no_response_after !== undefined) {
    ...
    if (!Number.isInteger(count) || count < 1 || count > 100) throw validationError("close_no_response_after must be between 1 and 100");
    result.closeNoResponseAfter = count;
  }
  if (body.suggest_follow_up !== undefined) {
    if (typeof body.suggest_follow_up !== "boolean") throw validationError("suggest_follow_up must be boolean");
    result.suggestFollowUp = body.suggest_follow_up;
  }
  if (body.require_next_step !== undefined) {
    if (typeof body.require_next_step !== "boolean") throw validationError("require_next_step must be boolean");
    result.requireNextStep = body.require_next_step;
  }
  if (!Object.keys(result).length) throw validationError("At least one lead setting is required");
  return result;
}
