...
import { parseAssignment, parseCreateLead, parseLeadListFilters, parseLeadSettings, parseRecordResult } from "./contracts";
...
describe("lead HTTP contracts", () => {
  it("normalizes a minimal create command and applies provider-neutral defaults", () => {
    expect(parseCreateLead({ contact_id: id, next_follow_up_at: "2026-08-21T12:00:00Z" })).toMatchObject({ contactId: id, source: "manual", priority: "normal", nextFollowUpAt: "2026-08-21T12:00:00.000Z" });
  });

  it("rejects account_id supplied in place of required domain fields", () => {
    expect(() => parseCreateLead({ account_id: id, next_follow_up_at: "2026-08-21" })).toThrow(LeadDomainError);
  });

  it.each(["no_answer", "contacted", "qualified", "rescheduled"])("requires a next action for %s", (result) => {
    expect(() => parseRecordResult({ channel: "whatsapp", result })).toThrow(/next_follow_up_at/);
  });
  ...
  it("bounds pagination and parses unassigned", () => {
    expect(parseLeadListFilters(new URLSearchParams("page=2&limit=50&assigned_to=unassigned"))).toMatchObject({ page: 2, limit: 50, assignedToUserId: null });
    expect(() => parseLeadListFilters(new URLSearchParams("limit=101"))).toThrow(/between 1 and 100/);
  });

  it("accepts only known lead settings", () => {
    expect(parseLeadSettings({ close_no_response_after: 5, require_next_step: true })).toEqual({ closeNoResponseAfter: 5, requireNextStep: true });
